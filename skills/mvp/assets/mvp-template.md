...
> - **Stories** are task-list one-liners indented under their epic: each independently shippable, verifiable, small enough for one PR, and tickable as work lands. Lead with an action verb — "Add …", "Ship …", "Template …" — and stay within two sentences.
> - **E1 is usually a walking skeleton built around the core primitive**: the first stories make the hero loop run end to end in its thinnest form (start the app, see hello world), the closing stories ship it through one real distribution channel — proving build and release pipes early while the core stays first. Remaining channels are a later epic, and every later epic ships through pipes that already work.
